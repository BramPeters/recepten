...
// JSON output voor een jQuery eindtaakAjaxcall, geen controller
//we gebruiken dit script enkel indien geen db voorhanden is
...
foreach ($countries as $key => $cn) {
    $land = array("country_code" => $key, "country_name"=> $cn);
	array_push($landen,$land);
}

if (!isset($_GET['country_code'])){
	$output = json_encode($landen);
	//$output = json_encode($countries);
	}
else{
	if (array_key_exists($country_code, $countries)) {
    	$output = json_encode($countries[$country_code]);
	}
	else{
		$output = "{}";
	}
